...
def add_product(name: str, category: str, price: float, 
    ...
    Если игра уже существует, выбрасывает ValueError.
    """
    conn = get_connection()
    try:
        cursor = conn.cursor()
        ...
        if existing:
            raise ValueError(f"Игра '{name}' уже существует в каталоге")
        ...
        # Пробрасываем ValueError как есть (для существующих игр)
        raise
    except Exception as e:
        conn.rollback()

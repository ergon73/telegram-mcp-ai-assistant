...
ВАЖНО: Если нужен инструмент, вызывай его СРАЗУ в формате JSON, БЕЗ текста перед ним.
Не пиши "Давайте я покажу..." или "Один момент..." перед вызовом инструмента.
...
Запрос: "игры дешевле 20 рублей"
Ответ: {"tool": "find_products_by_price_range", "arguments": {"min_price": 0, "max_price": 20}}

Запрос: "игры от 20 до 50 долларов"
Ответ: {"tool": "find_products_by_price_range", "arguments": {"min_price": 20, "max_price": 50}}

Запрос: "игры дороже 50"
Ответ: {"tool": "find_products_by_price_range", "arguments": {"min_price": 50, "max_price": 1000}}
...
Ответ: {"tool": "list_featured_products", "arguments": {}}

Запрос: "сколько будет 123 * 456"
Ответ: {"tool": "calculate", "arguments": {"expression": "123 * 456"}}

Если инструмент не нужен (например, пользователь просто поздоровался), отвечай обычным текстом.
...
async def process_user_message(message: Message) -> None:
    ...
    if not user_text:
        await message.answer("Отправьте текстовое сообщение, чтобы я смог помочь 👍")
        return
    ...
    if tool_call and "tool" in tool_call:
        ...
        # Логирование для отладки поиска по цене
        if tool_name == "find_products_by_price_range":
            print(f"[DEBUG] Price range search: min_price={arguments.get('min_price')}, max_price={arguments.get('max_price')}")
        ...
        if tool_name == "find_products_by_price_range":
            if result.get("ok") and isinstance(result.get("result"), list):
                print(f"[DEBUG] Found {len(result.get('result', []))} games in price range")
        ...
        if result.get("ok", False):
            ...
            if isinstance(tool_result, list) and tool_result:
                ...
                if "name" in tool_result:
                    ...
                    # Определяем, это добавление новой игры или просто показ существующей
                    if tool_name == "add_product":
                        formatted_result = (
                            f"✅ Игра успешно добавлена в каталог!\n\n"
                            f"{featured_mark}{product['name']}\n"
                            f"Платформа: {product['platform']}\n"
                            f"Жанр: {product['category']}\n"
                            f"Цена: {product['price']} ₽"
                ...
                if tool_name == "calculate":
                    # Извлекаем выражение из аргументов для более дружелюбного ответа
                    expression = arguments.get("expression", "")
                    if expression:
                        formatted_result = f"Результат: {formatted_result}"
                    else:
                        formatted_result = f"Результат вычисления: {formatted_result}"
            
            # Для остальных типов результатов отправляем сразу, без дополнительного вызова GPT
            # Это предотвращает дублирование ответов
            add_to_context(user_id, "assistant", formatted_result)
            await message.answer(formatted_result)
            return
        else:
